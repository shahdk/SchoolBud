...
public class GradeTrendGraphTest {
	...
	public void testIntegrationGradeTrendGraph1() throws ParseException {
		...
		GradeTrendGraph graph = new GradeTrendGraph(course, 5, 4);
		graph.setStartDate(start);
		graph.setEndDate(end);
		...
		assertEquals(graph.getGradePredictionCurvePoints().get(0).getY(), 56,
				0.5);
		assertEquals(graph.getGradePredictionCurvePoints().get(1).getY(), 74,
				0.5);
		assertEquals(graph.getGradePredictionCurvePoints().get(2).getY(), 86,
				0.5);
		assertEquals(graph.getGradePredictionCurvePoints().get(3).getY(), 89,
				0.5);
