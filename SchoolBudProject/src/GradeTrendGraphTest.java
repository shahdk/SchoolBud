...
public class GradeTrendGraphTest {
	...
	public void testIntegrationGradeTrendGraph() throws ParseException {
		...
		// TEST DATES
		ArrayList<Date> dates = this.findDates(newDt3, newDt2);

		// check points X - date
		assertEquals(graph.getGradePredictionCurvePoints().get(0).getX(),
				newDt3);
		System.out.println("1--- " + graph.getGradePredictionCurvePoints().get(0).getX());
		System.out.println("1--- " + newDt3);
		assertEquals(graph.getGradePredictionCurvePoints().get(1).getX(),
				dates.get(1));
		System.out.println("2--- " + graph.getGradePredictionCurvePoints().get(1).getX());
		System.out.println("2--- " + dates.get(1));
